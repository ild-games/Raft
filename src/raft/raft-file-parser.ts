...
import { CMakeDependency, Dependency, RaftDependency } from "./core/dependency";
...
} from "./core/raft-file-descriptor";
import { GitRepository, Repository } from "./core/vcs";
...
export function createRepository(
  repoDescriptor: RepositoryDescriptor
): Repository {
 ...
 */
export function getSupportedArchitectures(
  supportedArchitectures: ArchitectureDescriptor[]
  ...
  for (let architecture of supportedArchitectures) {
    let foundPlatform = platforms.find(
      (p) => p.name.toUpperCase() === architecture.platform.toUpperCase()
    );
    ...
    let foundArchitecture = foundPlatform
      ...
      .find(
        (arch) =>
          arch.name.toUpperCase() === architecture.architecture.toUpperCase()
      );

    if (!foundArchitecture) {
      throwConfigurationError(
        `unrecognized architecture ${architecture.architecture} for platform ${architecture.platform}`
      );
    }
  ...
  if (supported.length === 0) {
    ...
    supported.push({
      platform,
      architecture: platform.getDefaultArchitecture(),
    });
  }
