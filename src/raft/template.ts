...
export function instantiateTemplate(
  ...
  return new Promise((resolve, reject) => {
    var stream = gulp
      .src([
        templatePath.append("**").toString(),
        "!" + templatePath.append("index.js").toString(),
      ])
      .pipe(gulpif(new RegExp("!**/*.png"), mustache(context)))
      .pipe(
        rename(function (parsedpath: ParsedPath) {
 ...
 */
export async function instantiateRepositoryTemplate(
  repo: Repository,
  destination: Path,
  context: any
) {
...
function renameFile(parsedpath: ParsedPath, context: any) {
  for (var key in context) {
    parsedpath.dirname = replaceAll(
      parsedpath.dirname,
      `__${key}__`,
      context[key]
    );
    parsedpath.basename = replaceAll(
      parsedpath.basename,
      `__${key}__`,
      context[key]
    );
    parsedpath.extname = replaceAll(
      parsedpath.extname,
      `__${key}__`,
      context[key]
    );
